...
import {IonicAuthFirebaseUIConfig} from '../interfaces/config.interface';
...
import {IonicAuthFirebaseUIConfigToken} from '../ionic-auth-firebaseui.module';
...
export class AuthProcessService implements ISignInProcess, ISignUpProcess {
  ...
  async handleSuccess(userCredential: UserCredential) {
    ...
    if (this.config.toastMessageOnAuthSuccess) {
      const toast = await this.toastController.create({
        message: this.messageOnAuthSuccess ? this.messageOnAuthSuccess :
          `Hi ${userCredential.user.displayName ? userCredential.user.displayName : ''}!`,
        duration: 5000
      });
      toast.present();
    }
  ...
  async handleError(error: any) {
    ...
    if (this.config.toastMessageOnAuthError) {
      const toast = await this.toastController.create({
        message: this.messageOnAuthError ? this.messageOnAuthError :
          error.message,
        duration: 5000
      });
      toast.present();
