...
const config = {
  libraryName: 'ionic-auth-firebaseui',
  unscopedLibraryName: 'ionic-auth-firebaseui',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ionic-auth-firebaseui/ --deploy-url /ionic-auth-firebaseui/`, {cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ionic-auth-firebaseui-demo:server`, {cwd: `${config.demoDir}`}))
